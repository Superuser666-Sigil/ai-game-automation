...
Analyzes your training data and system capabilities to recommend the optimal
model configuration. Run this after 4_analyze_data_quality.py to make informed
decisions about model size.
"""
...
def check_data_exists():
    ...
    data_paths = [
        ...
        (
            "data_human/actions.npy",
            "data_human/frames",
        ),  # data_human directory
    ]
...
def analyze_data_complexity():
    ...
    try:
        ...
        print(
            f"   • Frame-to-frame changes: {key_changes:.3f} (keys), "
            f"{mouse_changes:.3f} (mouse)"
        )
...
def calculate_model_size(
    ...
    # LSTM parameters
    lstm_params = lstm_layers * (
        4
        * (cnn_output_size * lstm_hidden + lstm_hidden * lstm_hidden + lstm_hidden * 2)
    )

    # Head parameters
    head_params = (
        (lstm_hidden * 128 + 128)
        + (128 * COMMON_KEYS_COUNT + COMMON_KEYS_COUNT)
        + (lstm_hidden * 64 + 64)
        + (64 * 2 + 2)
        + (lstm_hidden * 32 + 32)
        + (32 * 2 + 2)
    ...
    sequence_length = 5
    batch_memory_mb = (
        batch_size * 3 * img_height * img_width * sequence_length * 4
    ) / (1024 * 1024)
...
def recommend_configuration(data_complexity, system_caps):
    ...
    final_rec = min(
        [complexity_rec, system_rec, data_amount_rec],
    ...
    print(
        f"   • Input Resolution: {recommended_config['img_size'][0]}x{recommended_config['img_size'][1]}"
    )
    print(
        f"   • LSTM: {recommended_config['lstm_hidden']} hidden, {recommended_config['lstm_layers']} layers"
    )
    ...
    print(
        f"   • Estimated training time: {estimate_training_time(final_rec, system_caps['gpu_type'])}"
    )

    # Memory check
    required_memory_gb = size_info["training_memory_mb"] / 1024
    if (
        required_memory_gb > system_caps["ram_gb"] * 0.8
...
def show_all_configurations():
    ...
    print(
        f"{'Configuration':<18} {'Resolution':<12} {'File Size':<10} {'RAM Usage':<11} {'Training Time':<15}"
    )
    print("-" * 80)
    ...
    for name, w, h, hidden, layers, cnn, batch in configs:
        ...
        model_size = (
            "small" if "Small" in name else ("large" if "Large" in name else "medium")
        )
        time_est = estimate_training_time(model_size, gpu_type)

        print(
            f"{name:<18} {w}x{h:<7} {size_info['file_size_mb']:<9.1f}MB "
...
def main():
    ...
    recommended_config, size_info = recommend_configuration(
        data_complexity, system_caps
    )
